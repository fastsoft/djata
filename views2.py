        ...
        if not (
            ...
            if hasattr(meta, 'model'):
                ...
                if not hasattr(meta, 'verbose_name'):
                    meta.verbose_name = model._meta.verbose_name.__str__()
                if not hasattr(meta, 'verbose_name_plural'):
                    meta.verbose_name_plural = model._meta.verbose_name_plural.__str__()
            else:
                if not hasattr(meta, 'verbose_name'):
            ...
            views.model_views[meta.verbose_name_plural] = self
            self.views = views
    ...
    def respond(
        ...
        if model_name is not None:
            ...
            if model_name in lookup:
                view, responder = lookup[model_name]
            ...
            raise NoSuchViewError(model_name)
        else:
            return self(request)
...
def respond(
    ...
    try:
        try:
            module = __import__(module_name, {}, {}, [model_name])
            views = module.views
